...
With **Settings → Learning** turned on, Lint notices writing habits you correct (a word you misspell, an article you drop), remembers them as short rules and uses them from the next suggestion on. While it is off, Lint behaves exactly as before. The few memories that fit what you are writing (at most five short lines) are added to the end of the prompt sent to your model; with a local model that stays on your Mac.
...
在「設定 → 學習」開啟後，Lint 會留意你修正過的寫作習慣（拼錯的字、漏掉的冠詞），記成簡短的規則，從下一次建議就開始使用。關閉時，Lint 的行為與以前完全相同。符合你正在寫的內容的少數記憶（最多五行短句）會加在送給模型的 prompt 結尾；使用本機模型時，這些都留在你的 Mac 上。
...
- **學到就用，證明有用才留下。** 新學到的記憶是短期記憶，下一次建議就會用到。7 天內同樣的樣式又出現（你又做了同樣的修正，或模型照提醒改了而你接受），它就轉成長期記憶；沒有的話就忘記：不再使用，只在背景留下痕跡，之後如果再出現，會被認出來並直接成為長期記憶。模型改的、要看句子而定的修正（換成另一個字，或同一個字的另一種形式，例如 *tickets* 改成 *ticket*）要再出現一次才會使用。你推翻某條記憶要求的改法時，除非它已經很穩固，否則會立刻忘記。長期記憶不再出現就會淡出：超過一個月沒出現後，證據每三個月減半（一般規則每六個月、核心記憶每一年），低於起始的一半就忘記。淡掉的痕跡會從資料庫永久刪除，大約是最後一次出現後三到十一個月。已釘選與已停用的記憶不會淡出，它們和你改寫過的記憶都不會被刪除；按「啟用」可以把忘記的記憶找回來。
- **記憶整理**（「設定 → 學習 → 記憶整理」），類似作夢：Lint 會記下哪些記憶證明有用、哪些要忘記，刪除已經淡掉的痕跡，並把同一類的多條記憶歸納成一條一般規則（目前只處理：你在不同動詞後反覆刪掉的介系詞，例如「discuss about」與「mention about」）。規則由固定模板寫成，不會含有任何你寫過的字。被歸納的記憶都會保留：規則使用中時它們標示為「已由一般規則涵蓋」並暫時不進 prompt，規則淡出、停用或被刪除後就自動恢復；文字裡出現它自己那個字時，它仍會單獨提醒。已釘選、已停用與你手動改寫過的記憶不會被歸納，你刪掉的規則不會再被重新產生，持續數週都證明有用的規則會升為核心記憶。整理只在這台 Mac 上進行（不連雲端、不下載模型），會等到你不在等建議時才開始；Lint 使用期間至少每天自動整理一次（啟動後一分鐘，或當天第一次回饋後），累積足夠新學習後也會整理，也可按「立即整理記憶」，完成後會顯示記住、忘記與刪除了幾條。
- 在「設定 → 學習 → 管理記憶」可以查看、改寫、釘選、停用或刪除每一條記憶，看到它是短期、長期還是已忘記（已忘記的要切換才會列出）、是具體、一般還是核心，以及規則由哪些記憶歸納而來，也能全部清除或重設整個資料庫。
