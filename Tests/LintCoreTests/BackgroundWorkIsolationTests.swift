...
/// Memory and dreaming run on their own, in the background, whenever the user
...
final class BackgroundWorkIsolationTests: XCTestCase {
    ...
    func testNothingInMemoryOrDreamingCanReachLocalAI() throws {
        let files = try swiftFiles(under: "Sources/LintCore/Memory")
        XCTAssertGreaterThan(files.count, 10, "the scan found the Memory sources")
        for file in files {
            let source = try String(contentsOf: file, encoding: .utf8)
            for name in forbidden {
                XCTAssertFalse(
                    source.contains(name),
                    "\(file.lastPathComponent) mentions \(name): background memory work must never start, install or wake local AI"
                )
            }
        }
    }

    /// Everything under Memory imports is on-device: the database, Apple's on-device language
    /// tokenizer, hashing. Nothing that can open a connection or load a model.
    func testMemoryOnlyImportsLocalThings() throws {
        let allowed: Set<String> = ["Foundation", "CryptoKit", "GRDB", "NaturalLanguage", "SQLite3", "os"]
        for file in try swiftFiles(under: "Sources/LintCore/Memory") {
            let source = try String(contentsOf: file, encoding: .utf8)
