...
final class WritingEvalFixtureTests: XCTestCase {
    func testTheFixturesCoverLintsWorkloadAndAreWellFormed() throws {
        ...
        XCTAssertGreaterThanOrEqual(fixtures.cases.count, 60, "the set is meant to be 60-120 cases")
        XCTAssertLessThanOrEqual(fixtures.cases.count, 120)
        ...
        for wanted in [
            "english-grammar", "english-minimal-edit", "zh-hant-proofread", "taiwan-wording",
            "en-to-zh-hant", "zh-hant-to-en", "tone-formal", "tone-concise", "tone-professional",
            "numbers-dates", "urls", "names", "punctuation", "mixed-language", "bullet-list",
        ...
        for testCase in fixtures.cases {
            ...
            if testCase.writingMode == .translate {
                XCTAssertNotNil(testCase.translateTarget, "\(testCase.id): a translation needs a target")
            }
            ...
            XCTAssertFalse(testCase.systemPrompt(profile: .onDevice).isEmpty)
        }
        XCTAssertGreaterThanOrEqual(
            fixtures.cases.filter { $0.expectUnchanged == true }.count, 20,
