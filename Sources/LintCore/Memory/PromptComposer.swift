...
/// Adds the user's remembered habits to the end of a system prompt. The end, because a local model
...
enum PromptComposer {
    ...
    /// it has to be while memory is off. Memories keep their order, within the prompt budget;
    ...
    static func compose(base: String, memories: [WritingMemory], english: Bool = false) -> PersonalizedPrompt {
        ...
        for memory in memories {
            guard used.count < MemoryPolicy.maxPersonalizedMemories else { break }
            let cost = MemoryPolicy.promptCost(of: memory)
