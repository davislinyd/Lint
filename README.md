...
Live check — watching the focused field and preparing a suggestion while you type — is on by default. Turn it off in Settings → General.

### Permissions

System Settings → Privacy & Security → Accessibility → enable **Lint**. Accessibility can read text in the app you are using, including the focused field. Lint reads the text you select, the field `⌥⌘K` checks, and, while live check is on (the default), the field you are typing in.
...
> **Preview builds:** the Pre-release entries on the Releases page (`Lint-<version>-macOS-arm64-preview.dmg`) are unsigned previews from before the first notarized release; install the latest notarized release instead. If you do open a preview, macOS blocks it the first time: go to System Settings → Privacy & Security and click **Open Anyway**, or run `xattr -dr com.apple.quarantine /Applications/Lint.app`. Allow Accessibility again after installing each new preview.
...
- **Downloaded after you agree:** the GGUF model. It is not in the DMG. It is checked against a pinned SHA-256 and stored under `~/Library/Application Support/Lint/Models`; you can remove it in Settings → Model. Apart from this download, the default setup uses the network only to check GitHub for Lint updates (see [Updates](#updates)).
...
Settings → Model → Advanced lets you point Lint at another `llama-server` (a Homebrew or self-built one), or at a Hugging Face model that `llama-server` downloads itself (`-hf`). You can also choose an OpenAI-compatible endpoint (Ollama, LM Studio, custom) as the model source. None of this is needed for the default setup.

### Requirements

- macOS 14+
- To build from source: Apple's Command Line Tools with a Swift 6.1+ toolchain (Xcode also works)
- An LLM backend: by default nothing extra, since Lint bundles llama.cpp and downloads its own model. Alternatively Apple Intelligence (macOS 26 or later) or an OpenAI-compatible endpoint (default `http://127.0.0.1:8000/v1`, e.g. `llama-server`, Ollama, LM Studio)
...
- Model source: Automatic, Apple Intelligence, local llama.cpp (managed by Lint) or an OpenAI-compatible endpoint (Ollama / LM Studio / custom). OpenAI, Anthropic and Google Gemini are listed but greyed out until they have been tested, and the ChatGPT (Plus / Pro login) source is turned off.
...
Lint checks GitHub Releases for a newer official release: every week by default (Settings → General → Updates: every launch, day, week or month), and whenever you choose **Check for Updates…** in the menu bar menu.

- **Ask before installing** (the default): Lint shows the new version, and **Download and Install** installs it.
- **Install automatically**: Lint installs it once the suggestion window is closed, then restarts.
- **Only check**: Lint only tells you about it.

A check only asks the GitHub API for the latest release; none of your text is sent. Before anything is replaced, the downloaded DMG has to match the published SHA-256, and the app inside it has to be Lint (`app.lint.assistant`) signed with Lint's Developer ID and Hardened Runtime. The new copy is swapped in after Lint quits, and only at `/Applications/Lint.app` or `~/Applications/Lint.app`. A copy anywhere else, or one not signed with Lint's Developer ID (such as a source build), can check for updates but is not replaced.
...
With **Settings → Memory** turned on, Lint notices writing habits you correct (a word you misspell, an article you drop), writes them down as short rules and uses them from the next suggestion on. While it is off, Lint behaves exactly as before. The few memories that fit what you are writing (at most five short lines) are added to the end of the prompt sent to your model; with a local model that stays on your Mac.

- Everything stays on your Mac, in `~/Library/Application Support/Lint/LintLearning.sqlite` (readable only by you). The settings page calls this Memory; the file name stays `LintLearning.sqlite`.
- Only abstracted rules are kept: a word, a short phrase or a template sentence, never the text around it. Numbers, addresses, paths, acronyms and capitalised names are left out. Lint cannot recognise Chinese names or names typed in lowercase, so a name you corrected can still end up in a memory; you can read and delete every memory under Settings → Memory → Manage Memories.
- What you did with a suggestion (replace, copy, rewrite) is logged as keyed hashes, not text; the key is in the Keychain.
- **Written down at once, kept only if it proves itself.** A new memory is short-term and goes into the next prompt. If its pattern comes back within 7 days (you make the same fix again, or the model applies the reminder and you accept that), it becomes long-term; if not, it is forgotten: no longer used, and kept only as a trace, so that it is recognised, and long-term at once, if it does come back. A change the model made that depends on its sentence (another word, or the same word in another form such as *tickets* for *ticket*) is used only once it has come back. Undoing what a memory asks for forgets it at once unless it is well established. A long-term memory that stops showing up fades: after a month without it the evidence halves every three months (every six for a general rule, every year for a core one), and below half of where it started it is forgotten. A trace that has faded away is erased from the file for good, roughly three to eleven months after the pattern was last seen. Pinned and disabled memories do not fade, neither they nor memories you reworded are ever erased, and Enable brings a forgotten memory back.
- **Dreaming** (Settings → Memory → Dreaming): Lint writes down which memories proved themselves and which are forgotten, erases traces that have faded, and sums up several memories of one kind in one general rule (for now: a preposition you keep deleting after different verbs, such as "discuss about" and "mention about"). The rule is worded from a fixed template, so it never contains a word you wrote. The memories behind it are kept: they are marked as covered and left out of the prompt while the rule is in use, and they come back on their own once the rule fades, is disabled or is deleted; one whose own words are in your text still speaks for itself. Pinned, disabled and hand-edited memories are never combined, a rule you delete is not made again, and a rule that keeps proving itself for weeks becomes a core memory. It runs only on this Mac (no cloud, no model download), waits until you are not waiting on a suggestion, and starts by itself at least once a day while Lint is in use (a minute after start-up, or after the first feedback of the day), after enough new memories, or when you press Dream Now, which then shows how many memories it remembered, forgot and erased.
- Under Settings → Memory → Manage Memories you can read, reword, pin, disable or delete every memory, see whether it is short-term, long-term or forgotten (forgotten ones are listed only when you ask), whether it is specific, general or core and which memories a rule was derived from, clear them all, or reset the whole database.
...
即時檢查（打字時監看焦點欄位並在背景準備建議）預設開啟，可在「設定 → 一般」關閉。

### 權限

系統設定 → 隱私權與安全性 → 輔助功能 → 勾選 **Lint**。輔助功能可以讀取你正在使用的 App 裡的文字，包含焦點欄位。Lint 會讀你選取的文字、`⌥⌘K` 檢查的欄位，以及即時檢查開著時（預設開）你正在輸入的欄位。
...
> **預覽版：** Releases 頁上標示為 Pre-release 的項目（`Lint-<版本>-macOS-arm64-preview.dmg`）是第一個公證版發佈之前的未簽章預覽版；請改裝最新的公證版。如果還是要開預覽版，第一次開啟時 macOS 會擋下：到 系統設定 → 隱私權與安全性 按「仍要打開」，或執行 `xattr -dr com.apple.quarantine /Applications/Lint.app`。每安裝一個新的預覽版，輔助功能都要重新允許。
...
- **同意後才下載：**GGUF 模型。它不在 DMG 裡，下載後會以固定的 SHA-256 驗證，存放在 `~/Library/Application Support/Lint/Models`，可在「設定 → 模型」移除。除了這次下載，預設設定只會用網路到 GitHub 檢查 Lint 的更新（見〈更新〉）。
...
「設定 → 模型 → 進階」可以改用另一個 `llama-server`（Homebrew 或自行編譯的版本），或改用由 `llama-server` 自己下載的 Hugging Face 模型（`-hf`）。也可以把 OpenAI 相容端點（Ollama、LM Studio、自訂）當作模型來源。預設設定完全用不到這些。

### 需求

- macOS 14+
- 從原始碼建置：Apple 的 Command Line Tools，含 Swift 6.1+ 工具鏈（裝 Xcode 也可以）
- LLM 後端：預設不需要額外安裝，Lint 內建 llama.cpp 並自行下載模型。也可以改用 Apple Intelligence（macOS 26 以上）或 OpenAI 相容 API（預設 `http://127.0.0.1:8000/v1`，例如 `llama-server`、Ollama、LM Studio）
...
- 模型來源：自動、Apple Intelligence、本機 llama.cpp（由 Lint 管理），或 OpenAI 相容端點（Ollama / LM Studio / 自訂）。OpenAI、Anthropic、Google Gemini 會列出，但在測試過之前是灰色、不能選；ChatGPT（Plus／Pro 登入）來源已關閉。
- AI 引擎：**自動**、**Apple Intelligence** 或 **Lint 本機 AI**。Apple Intelligence 使用 macOS 26 以上內建的裝置端模型（不必另外下載；Lint 只用裝置端模型，從不使用 Private Cloud Compute）。「自動」在 Apple Intelligence 可用時使用它，否則使用已經設定好的 Lint 本機 AI；它不會自己開始下載。升級時保留原有設定。與 Gemma 的比較見 [docs/APPLE-INTELLIGENCE.md](docs/APPLE-INTELLIGENCE.md)。
...
Lint 會到 GitHub Releases 檢查有沒有較新的正式版：預設每週一次（「設定 → 一般 → 更新」可選每次啟動、每天、每週或每月），在選單列選單按**「檢查更新…」**也會立即檢查。

- **手動更新**（預設）：Lint 顯示新版本，按**「下載並安裝」**才安裝。
- **自動更新**：建議視窗關閉後就安裝，並重新啟動 Lint。
- **只檢查**：只告訴你有新版本。

檢查時只向 GitHub API 查詢最新的正式版，不會送出你的任何文字。換上新版之前，下載的 DMG 必須符合公布的 SHA-256，裡面的 App 必須是 Lint（`app.lint.assistant`），並以 Lint 的 Developer ID 簽署、啟用 Hardened Runtime。新版在 Lint 結束後才換上，而且只會取代 `/Applications/Lint.app` 或 `~/Applications/Lint.app`。放在其他位置、或不是以 Lint 的 Developer ID 簽署的 Lint（例如從原始碼建置的版本），可以檢查更新，但不會被取代。
...
- **模型：**可在「設定 → 模型」選擇 Gemma 4 E4B（預設，約 5.2 GB）、Qwen3 4B（選用，2.5 GB）或 Gemma 4 12B（選用，7 GB）。Gemma 4 12B 標示為有記憶體風險：在 16 GB 的 Mac 上可能讓系統變得非常慢或沒有回應。切換模型不會刪除任何模型，已經有的模型也不會重新下載。升級會保留你已安裝的模型；只把 12B 存成舊預設、卻從未下載過的安裝會改用 E4B。
- **調校與閒置釋放：**Lint 給每個由它管理的模型各自的 llama-server 參數（context 3072、q8_0 KV cache，Gemma 4 另加 `--reasoning off`）；進階的「額外參數」欄位預設是空的，只會覆蓋它寫到的選項。5 分鐘沒有要求建議時（「設定 → 模型 → 記憶體」），llama-server 會釋放模型佔用的記憶體，下一次要求時再重新載入，約需幾秒。升級時會清掉舊版內建預設參數字串的存檔；你自己輸入的內容會保留。
...
在「設定 → 記憶」開啟後，Lint 會留意你修正過的寫作習慣（拼錯的字、漏掉的冠詞），記成簡短的規則，從下一次建議就開始使用。關閉時，Lint 的行為與以前完全相同。符合你正在寫的內容的少數記憶（最多五行短句）會加在送給模型的 prompt 結尾；使用本機模型時，這些都留在你的 Mac 上。

- 全部只存在你的 Mac：`~/Library/Application Support/Lint/LintLearning.sqlite`（僅你本人可讀）。設定頁現在叫記憶，檔名仍是 `LintLearning.sqlite`。
- 只保存抽象後的規則（一個字、一小段詞組或模板句），不保存周圍的文字。數字、網址、路徑、縮寫與大寫開頭的人名不會被記下。Lint 認不出中文人名或全小寫的英文名，被你修正過的名字仍可能進入記憶；可在「設定 → 記憶 → 管理記憶」查看並刪除。
- 你對建議做了什麼（取代、複製、重寫）只以帶金鑰的雜湊記錄，不含文字；金鑰放在鑰匙圈。
- **記下就用，證明有用才留下。** 新記下的記憶是短期記憶，下一次建議就會用到。7 天內同樣的樣式又出現（你又做了同樣的修正，或模型照提醒改了而你接受），它就轉成長期記憶；沒有的話就忘記：不再使用，只在背景留下痕跡，之後如果再出現，會被認出來並直接成為長期記憶。模型改的、要看句子而定的修正（換成另一個字，或同一個字的另一種形式，例如 *tickets* 改成 *ticket*）要再出現一次才會使用。你推翻某條記憶要求的改法時，除非它已經很穩固，否則會立刻忘記。長期記憶不再出現就會淡出：超過一個月沒出現後，證據每三個月減半（一般規則每六個月、核心記憶每一年），低於起始的一半就忘記。淡掉的痕跡會從資料庫永久刪除，大約是最後一次出現後三到十一個月。已釘選與已停用的記憶不會淡出，它們和你改寫過的記憶都不會被刪除；按「啟用」可以把忘記的記憶找回來。
- **作夢**（「設定 → 記憶 → 作夢」）：Lint 會記下哪些記憶證明有用、哪些要忘記，刪除已經淡掉的痕跡，並把同一類的多條記憶歸納成一條一般規則（目前只處理：你在不同動詞後反覆刪掉的介系詞，例如「discuss about」與「mention about」）。規則由固定模板寫成，不會含有任何你寫過的字。被歸納的記憶都會保留：規則使用中時它們標示為「已由一般規則涵蓋」並暫時不進 prompt，規則淡出、停用或被刪除後就自動恢復；文字裡出現它自己那個字時，它仍會單獨提醒。已釘選、已停用與你手動改寫過的記憶不會被歸納，你刪掉的規則不會再被重新產生，持續數週都證明有用的規則會升為核心記憶。作夢只在這台 Mac 上進行（不連雲端、不下載模型），會等到你不在等建議時才開始；Lint 使用期間至少每天自動作夢一次（啟動後一分鐘，或當天第一次回饋後），累積足夠的新記憶後也會作夢，也可按「現在作夢」，完成後會顯示記住、忘記與刪除了幾條。
- 在「設定 → 記憶 → 管理記憶」可以查看、改寫、釘選、停用或刪除每一條記憶，看到它是短期、長期還是已忘記（已忘記的要切換才會列出）、是具體、一般還是核心，以及規則由哪些記憶歸納而來，也能全部清除或重設整個資料庫。
